...
- `thread/delete` — hard-delete an active or archived thread and attempt to delete any spawned descendant threads; returns `{}` on success and emits `thread/deleted` for each deleted thread.
